-- Fix the ambiguous column reference in get_team_inbox_messages function
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tm.team_id = team_id_param AND tm.user_id = auth.uid()
  ) THEN
...
-- Also fix the simpler function in the same way
